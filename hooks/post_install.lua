...
function PLUGIN:PostInstall(ctx)
	...
	--[[
    os.execute("mkdir -p " .. path .. "/bin")
    os.execute("mv " .. path .. "/xcode-*/bin/* " .. path .. "/bin/")
	...
	--[[
    -- Archive already contains bin/xcode, just verify it works
    local testResult = os.execute(path .. "/bin/xcode --version > /dev/null 2>&1")
    if testResult ~= 0 then
        error("xcode installation appears to be broken")
    end
    --]]
